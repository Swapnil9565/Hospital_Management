...
export default function AddDoctor() {
  ...
  return (
    <div className="h-[90vh] pt-5" style={{ background: "radial-gradient(circle, rgba(63,94,251,1) 0%, rgb(14, 222, 249) 100%)" }}>
  
    <div className="max-w-lg mx-auto p-3 bg-white rounded-2xl shadow-lg">
      <h2 className="text-2xl font-semibold text-center mb-4">Add a New Doctor</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block font-medium">Upload Photo</label>
          <input type="file" accept="image/*" onChange={handleFileChange} className="mt-1 w-full border rounded-lg p-1" />
        </div>
        <div>
          <label className="block font-medium">Doctor Name</label>
          <input type="text" name="name" value={formData.name} onChange={handleChange} className="mt-1 w-full border rounded-lg p-1" required />
        </div>
        <div>
        ...
        <button type="submit" className="w-full bg-blue-500 text-white p-2 rounded-lg hover:bg-blue-600">Add Doctor</button>
      </form>
    </div>
